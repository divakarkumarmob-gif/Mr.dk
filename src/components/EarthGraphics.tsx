...
function Earth({ meshRef }: { meshRef: React.RefObject<THREE.Mesh> }) {
  ...
  return (
    <Sphere ref={meshRef} args={[0.8, 64, 64]} position={[0, 0, 0]} rotation={[0, 1.36, 0]}>
      <meshStandardMaterial map={textureMap || undefined} color={textureMap ? undefined : 'blue'} />
    </Sphere>
  );
}
...
function Moon() {
  ...
  React.useEffect(() => {
    new THREE.TextureLoader().load(
      ...
      setTextureMap,
      undefined,
      (err) => console.error("Moon texture failed to load", err)
    );
  }, []);
  ...
  return (
    <group ref={groupRef}>
      <Sphere ref={mesh} args={[0.3, 32, 32]} position={[1.5, 0, 0]}>
        <meshStandardMaterial map={textureMap || undefined} color={textureMap ? undefined : 'gray'} />
      </Sphere>
    </group>
  );
}
...
export default function EarthGraphics() {
  ...
  return (
    <div className="w-full h-48 sm:h-64 rounded-xl overflow-hidden shadow-lg mb-6">
