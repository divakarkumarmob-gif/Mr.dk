...
export default function CustomVideoPlayer({ src, title }: CustomVideoPlayerProps) {
  ...
  const onError = (e: React.SyntheticEvent<HTMLVideoElement, Event>) => {
    ...
    if (err) {
      switch (err.code) {
        case 1: message = 'MEDIA_ERR_ABORTED: Load aborted'; break;
        case 2: message = 'MEDIA_ERR_NETWORK: Network error while loading video (check CORS / connectivity)'; break;
        case 3: message = 'MEDIA_ERR_DECODE: Video decoding failed'; break;
        case 4: message = 'MEDIA_ERR_SRC_NOT_SUPPORTED: Source not supported or blocked (likely CORS/network block)'; break;
        default: message = `Unknown error code ${err.code}`;
      }
    }
    console.error('[CustomVideoPlayer] Video failed to load:', message, 'src:', src);
  ...
  return (
    ...
    >
      {/* Visible error state so failures are never a silent black screen */}
      {videoError ? (
        <div className="absolute inset-0 bg-[#0a0f1e] flex flex-col items-center justify-center text-center p-6 z-20">
          <p className="text-red-400 font-bold text-sm mb-2">Video failed to load</p>
          <p className="text-gray-400 text-xs break-all">{videoError}</p>
          <p className="text-gray-500 text-[10px] mt-3 break-all">URL: {src}</p>
        </div>
      ) : null}

      {/* Pulse Loading Skeleton for Blank state */}
      {!videoError && (!src || isLoading) ? (
