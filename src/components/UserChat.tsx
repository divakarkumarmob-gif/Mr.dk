...
export default function UserChat({ fullScreen, user, initialScreenshot, initialText }: { fullScreen?: boolean, user: any, initialScreenshot?: string, initialText?: string }) {
  ...
  return (
    <div className={`flex flex-col ${fullScreen ? 'h-dvh bg-[#0b141a]' : 'h-[500px] bg-[#0b141a] border border-white/10 rounded-xl shadow-2xl'} overflow-hidden`}>
      {/* WhatsApp-style header */}
      <div
        className="bg-[#1e293b] p-3 flex items-center gap-3 border-b border-white/10 flex-shrink-0"
        style={fullScreen ? { paddingTop: 'max(env(safe-area-inset-top, 0px), 12px)' } : undefined}
      >
