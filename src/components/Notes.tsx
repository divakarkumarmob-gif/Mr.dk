...
export default function Notes({ onNavigate }: { onNavigate: (view: any) => void }) {
  ...
  // Full-screen PDF viewer (uses the same in-app pdf.js renderer as
  // NCERT/PYQ/Question Bank instead of the Google Docs Viewer iframe,
  // which fails with "source server is preventing embedded display"
  // whenever Google's fetcher can't reach the Firebase Storage URL).
  ...
  const handleLivePhotoClick = async () => {
    if ((window as any).Capacitor) {
      try {
  ...
  return (
    <div id="notes-root" className="h-dvh text-slate-100 pb-44 px-3 overflow-y-auto">
                      ...
                      <div key={idx} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                        ...
                        >
                          <div className="flex flex-col items-center gap-2 text-slate-400">
                            ...
                            <span className="text-xs font-bold">Tap to open PDF</span>
                          </div>
                        </div>
                      </div>
                    );
