...
export default function TestResultDetail({ result, onBack }: { result: any, onBack: () => void }) {
    ...
    const handleShare = async () => {
        ...
        try {
            const dataUrl = await toPng(contentRef.current, { backgroundColor: '#0a0f24' });
            ...
            if (Capacitor.isNativePlatform()) {
                ...
                await shareResult("Test Result", "Check out my test result!", resultUri.uri);
            } else {
                // For web, we can't easily share images directly with navigator.share like this
                // but we can at least open in a new tab if needed or use download
    ...
    return (
        <div ref={contentRef} className="min-h-dvh bg-[#0a0f24] text-white p-6 pb-44 overflow-y-auto" style={{ paddingTop: 'max(env(safe-area-inset-top, 0px), 24px)' }}>
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
                ...
                <div className="bg-[#161e38] p-3 rounded-2xl text-center">
                    <div className="font-bold text-lg text-yellow-500">{timeTakenMin}:{timeTakenSec.toString().padStart(2, '0')}</div>
