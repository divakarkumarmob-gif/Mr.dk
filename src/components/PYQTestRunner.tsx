...
export default function PYQTestRunner(props: PYQTestRunnerProps) {
    ...
    const handleTestSubmit = async () => {
        ...
        // Per-chapter breakdown, used for weak-topic detection on the
        // dashboard. Only questions tagged with a chapter (from
        // PracticeTest / CustomPractice) contribute — untagged sets (e.g.
        // full mock tests without per-question chapter tags) simply won't
        // show up there, which is fine.
        const chapterStats: Record<string, { chapter: string; subject?: string; correct: number; incorrect: number; unattempted: number; total: number }> = {};
        questions.forEach(q => {
            if (!q.chapter) return;
            if (!chapterStats[q.chapter]) {
                chapterStats[q.chapter] = { chapter: q.chapter, subject: q.subject, correct: 0, incorrect: 0, unattempted: 0, total: 0 };
            }
            const stat = chapterStats[q.chapter];
            stat.total += 1;
        ...
        const topicAnalysis = Object.values(chapterStats).map(stat => ({
            ...stat,
    ...
    return (
        <div className="fixed inset-0 bg-white text-gray-900 font-sans z-[100] flex flex-col pt-[max(env(safe-area-inset-top,0px),5px)] px-3">
            ...
            {/* Header */}
            <div className="flex items-center justify-between px-2 py-4 border-b border-gray-100 bg-white sticky top-0 z-10">
