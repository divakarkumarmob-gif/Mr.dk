...
import { X, Sparkles, Mic, Brain, ExternalLink, Smartphone, CheckCircle, Copy } from 'lucide-react';
...
interface AppWidgetModalProps {
    isOpen: boolean;
    onClose: () => void;
    onLaunchTarget: (target: 'neural_solver' | 'liveAI') => void;
    isLoggedIn: boolean;
}
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                <motion.div
                    ...
                    className="relative w-full max-w-lg bg-[#0F172A] border border-blue-500/20 rounded-3xl p-6 shadow-2xl overflow-hidden text-white flex flex-col max-h-[90vh]"
                >
                    {/* Header */}
                    <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-white leading-snug">Home Screen Widgets</h2>
                                <p className="text-xs text-blue-300">1-Tap Quick Launchers for NEET Master AI</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="flex-grow overflow-y-auto custom-scrollbar space-y-4 pr-1">
                        {APP_WIDGETS.map((widget) => (
                            <div
                                key={widget.id}
                                className="group relative bg-slate-900/90 border border-white/10 hover:border-blue-500/40 rounded-2xl p-5 transition-all duration-200 hover:shadow-xl hover:shadow-blue-500/10"
                            >
                                <div className="flex items-start justify-between gap-4 mb-3">
                                    <div className="flex items-center gap-3.5">
                                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-900/50 to-purple-900/50 border border-white/10 flex items-center justify-center text-2xl shadow-inner shrink-0 group-hover:scale-105 transition-transform">
                                            {widget.icon}
                                        </div>
                                        <div>
                        ...
                        <div className="p-4 rounded-2xl bg-blue-950/40 border border-blue-500/20 text-xs text-blue-200 leading-relaxed space-y-2">
                            <div className="font-bold text-white flex items-center gap-2 text-sm">
                                ...
                                How to Add App Widgets to Android Home Screen
                            </div>
                            <ol className="list-decimal list-inside space-y-1.5 text-gray-300 pl-1">
                                ...
                                <li>Drag <b>🧠 Neural 2.0</b> or <b>🎙️ Live AI Voice</b> onto your Home Screen for 1-tap direct access!</li>
                            </ol>
                        </div>
                    </div>
