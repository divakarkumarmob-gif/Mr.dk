...
export default function PrivateVideos({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`fixed inset-0 bg-[#060a17] z-[1000] flex flex-col text-white font-sans overflow-hidden ${currentView === 'player' ? 'landscape:p-0 pt-[max(env(safe-area-inset-top,0px),12px)]' : 'pt-[max(env(safe-area-inset-top,0px),12px)] px-0 pb-[max(env(safe-area-inset-bottom,0px),12px)] landscape:px-0'}`}
    >
      {/* Dynamic Header */}
