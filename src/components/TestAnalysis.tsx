...
export default function TestAnalysis({ result, onClose }: TestAnalysisProps) {
    ...
    return (
        <motion.div 
            ...
            className="fixed inset-0 bg-[#060a18]/95 backdrop-blur-2xl text-slate-100 z-[120] flex flex-col p-4 sm:p-6 overflow-y-auto"
        >
            <div className="max-w-2xl mx-auto w-full flex-grow flex flex-col">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-800">
                    <button 
                        onClick={onClose} 
                        className="p-2.5 bg-slate-900 border border-slate-800 rounded-full hover:bg-slate-800 transition active:scale-95 text-slate-300 hover:text-white cursor-pointer"
                    >
                        <X className="w-5 h-5" />
                    </button>
                    <div className="flex items-center gap-2">
                        <Sparkles className="w-5 h-5 text-cyan-400 animate-pulse" />
                        <h2 className="font-extrabold text-lg text-white tracking-tight">AI Test Analysis</h2>
                    </div>
                    <div className="w-9" />
                </div>
                ...
                {loading ? (
                    <div className="flex-grow flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-slate-300 font-bold text-sm">Analyzing NEET performance metrics...</p>
                        <p className="text-slate-500 text-xs mt-1">Checking accuracy, speed, & subject weak spots</p>
                    </div>
                ) : (
                    <div className="space-y-5 flex-grow">
                        {deepAnalysis ? (
                            <div className="space-y-4">
                                <div className="space-y-1.5 bg-slate-900/90 p-5 rounded-2xl border border-rose-500/30 shadow-lg">
                                    ...
                                    <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line pt-1">{deepAnalysis.mistakes}</p>
                                </div>

                                <div className="space-y-1.5 bg-slate-900/90 p-5 rounded-2xl border border-cyan-500/30 shadow-lg">
                                    <h4 className="text-sm font-extrabold text-cyan-300 flex items-center gap-2">
                                        <Sparkles className="w-4 h-4 text-cyan-400" /> Kaise improve kare (Improvement Plan)
                                    </h4>
                                    <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line pt-1">{deepAnalysis.improvement}</p>
                                </div>

                                <div className="space-y-1.5 bg-slate-900/90 p-5 rounded-2xl border border-emerald-500/30 shadow-lg">
                                    ...
                                    <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line pt-1">{deepAnalysis.future}</p>
                                </div>
                            </div>
                        ) : (
                            <div className="bg-slate-900/90 p-6 rounded-3xl border border-slate-800 shadow-2xl backdrop-blur-md">
                                <h3 className="font-extrabold mb-4 flex items-center gap-2 text-cyan-300 text-sm tracking-wide uppercase">
                                    <BookOpen className="w-4 h-4 text-cyan-400" /> Detailed Mentor Insights
                                </h3>
                                <div className="text-xs text-slate-200 leading-relaxed whitespace-pre-line bg-slate-950/70 p-4.5 rounded-2xl border border-slate-800">
                                    {analysis}
                                </div>
                            </div>
                        )}
