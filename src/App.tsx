...
const CustomPractice = lazy(() => import('./components/CustomPractice'));
const PracticeTest = lazy(() => import('./components/PracticeTest'));
const PYQTestRunner = lazy(() => import('./components/PYQTestRunner'));
const HubSwitcher = lazy(() => import('./components/HubSwitcher'));
const VideoPlayer = lazy(() => import('./components/VideoPlayer'));
const Profile = lazy(() => import('./components/Profile'));
const SchoolSearch = lazy(() => import('./components/SchoolSearch'));
const NotesLibrary = lazy(() => import('./components/NotesLibrary'));
const EditProfile = lazy(() => import('./components/EditProfile'));
const AdminPanel = lazy(() => import('./components/AdminPanel'));
const AdminChatPage = lazy(() => import('./components/AdminChatPage'));
const TestHub = lazy(() => import('./components/TestHub'));
const Notes = lazy(() => import('./components/Notes'));
const AIStudyPlanPage = lazy(() => import('./components/AIStudyPlanPage'));
const NCERTHub = lazy(() => import('./components/NCERTHub'));
const NTAQuestionsHub = lazy(() => import('./components/NTAQuestionsHub'));
const OldPYQHistory = lazy(() => import('./components/OldPYQHistory'));
const Flashcards = lazy(() => import('./components/Flashcards'));
const StudyDashboard = lazy(() => import('./components/StudyDashboard'));
const PrivateVideosList = lazy(() => import('./components/PrivateVideosList'));
const PrivateVideosChapter = lazy(() => import('./components/PrivateVideosChapter'));
const PrivateVideosPlayer = lazy(() => import('./components/PrivateVideosPlayer'));
const TestResultDetail = lazy(() => import('./components/TestResultDetail'));
...
const MindHackPage = lazy(() => import('./components/MindHackPage'));
const NeuralSolver = lazy(() => import('./components/NeuralSolver'));
const LiveAIInterface = lazy(() => import('./components/LiveAIInterface'));
...
function AppInner() {
  ...
  const initializedNotificationsRef = React.useRef(false);
  ...
  useEffect(() => {
      ...
      configureStatusBar(lightViews.includes(currentView));
  }, [currentView]);

  // Keyboard resizing is handled automatically by @capacitor-community/safe-area
  // (see src/main.tsx). Manually calling Keyboard.setResizeMode here was found to
  // conflict with that plugin's internal resize logic and caused a black gap to
  ...
  useEffect(() => {
    ...
    const backButtonListener = CapacitorApp.addListener('backButton', ({ canGoBack }) => {
        ...
        if (activeVideoRef.current) { 
            ...
            const state = { ...window.history.state, activeVideo: null };
        ...
        const tabRoots = ['home', 'tests', 'analytics', 'notes', 'profile'];
        if (tabRoots.includes(currentView)) {
            if (currentView === 'home') {
                const now = Date.now();
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
            if (change.type === 'added') {
                // Note: we no longer fire a LocalNotifications.schedule() here.
                // The admin's /api/send-notification call already pushes a
                // real FCM message to every registered token (including the
                // sender's own device), so scheduling a second local
                // notification here just duplicates it and adds extra load
                // right when the admin taps Send.
  ...
  const [pendingSupportData, setPendingSupportData] = useState<{ screenshot: string, text: string } | null>(null);

  // Clear pending support screenshot/text once the user leaves the support chat,
  // whether via the system back button or in-app navigation.
  useEffect(() => {
    if (currentView !== 'technicalSupport' && pendingSupportData) {
      setPendingSupportData(null);
    }
  }, [currentView]);
  ...
  const [privateActiveVideo, setPrivateActiveVideo] = useState<any>(null);
    ...
    if (user) {
      const fetchPrivateVideos = async () => {
        ...
        try {
          ...
          setPrivateVideosSubjects(result.subjects || []);
          setPrivateVideosDebug(`OK status=${response.status} subjects=${(result.subjects || []).length}`);
        } catch (err: any) {
          console.error(err);
          setPrivateVideosDebug(`ERROR: ${err?.message || String(err)}`);
        }
      };
      fetchPrivateVideos();
    } else {
      setPrivateVideosDebug('no user (not logged in) - fetch skipped');
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      let guestUser = null;
      try {
          guestUser = await storageService.getItem('guest_user');
      } catch (e) {
          console.warn("Guest user key not found, ignoring.");
      }
      if (guestUser) {
        setLoading(false);
        return;
      ...
      if (currentUser) {
          ...
          // Keep displayName/email in sync on the users doc so admin-side chat
          // (and anywhere else reading from Firestore) can show the real name.
          if (currentUser.displayName || currentUser.email) {
              setDoc(doc(db, 'users', currentUser.uid), {
                  ...(currentUser.displayName && { displayName: currentUser.displayName }),
                  ...(currentUser.email && { email: currentUser.email }),
              }, { merge: true }).catch(e => console.error('Failed to sync displayName', e));
          }
  ...
  if (currentView === 'privateVideosChapter' && selectedSubject && selectedChapter) {
      return <PrivateVideosChapter selectedSubject={selectedSubject} selectedChapter={selectedChapter} onBack={() => setCurrentView('privateVideosList')} onNavigate={(vid) => {
  ...
  if (currentView === 'notes') {
    ...
    if (currentView === 'analysisResultDetail' && selectedAnalysisResult) {
        return <TestResultDetail result={selectedAnalysisResult} onBack={() => setCurrentView('analytics')} />;
    }
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <h1 className="sr-only">NeetMaster – AI-Powered NEET Exam Preparation App</h1>
      ...
      <Suspense fallback={null}>
        <WeakTopics onStartPractice={(chapters) => {
            setPracticeChapters(chapters);
            setCurrentView('practiceTest');
        }} />
      </Suspense>
